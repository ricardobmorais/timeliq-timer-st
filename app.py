...
# TABS
# ─────────────────────────────────────────
tab1, tab2, tab3, tab4 = st.tabs(["📝 Registar", "📋 Registos", "📊 Métricas", "⚙️"])
...
with tab1:
    ...
    if f and f.name not in st.session_state.get('importados', set()):
        txt = f.read().decode('windows-1252', errors='replace')
        add, skip = importar_csv(txt)
        if 'importados' not in st.session_state:
            st.session_state.importados = set()
        st.session_state.importados.add(f.name)
...
# Função para limpar duplicados (usar só uma vez)
def limpar_duplicados():
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                DELETE FROM timer_registos a
                USING timer_registos b
                WHERE a.id < b.id
                AND a.nopat = b.nopat
                AND a.data = b.data
                AND a.hora = b.hora
...
# ═══════════════════════════════
with tab4:
    st.markdown(f"<p style='font-weight:700;font-size:16px;color:{text}'>⚙️ Definições</p>", unsafe_allow_html=True)
    st.divider()

    st.markdown(f"<p style='color:{text};font-weight:600'>🗑 Apagar todos os registos</p>", unsafe_allow_html=True)
    st.warning("Esta ação apaga TODOS os registos da base de dados. Não é reversível.")
    
    confirmar = st.text_input("Escreve CONFIRMAR para apagar tudo", key="conf_delete")
    if st.button("🗑 Apagar tudo", key="btn_delete_all"):
        if confirmar == "CONFIRMAR":
            with get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute("DELETE FROM timer_registos")
                    conn.commit()
            st.success("✅ Todos os registos foram apagados.")
            st.rerun()
        else:
            st.error("Escreve CONFIRMAR (em maiúsculas) para confirmar.")

    st.divider()
    st.markdown(f"<p style='color:{text};font-weight:600'>🔁 Limpar duplicados</p>", unsafe_allow_html=True)
    st.caption("Remove registos duplicados com o mesmo PAT, data, hora início e hora fim.")
    if st.button("Limpar duplicados", key="btn_dedup"):
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    DELETE FROM timer_registos a
                ...
                deleted = cur.rowcount
                conn.commit()
        st.success(f"✅ {deleted} duplicado(s) removido(s).")
        st.rerun()
